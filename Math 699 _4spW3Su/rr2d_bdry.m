...
all_avg=[]; all_std=[]; % array to store the average distance and std
...
plot(R*cos(th),R*sin(th),'c--','LineWidth',3)
hold off

sz_all_avg = size(all_avg);

disp('average distant of rotor-router = '+all_avg(sz_all_avg(2)))
disp('std of rotor-router = '+all_std(sz_all_avg(2)))
...
legend({'IDLA','Rotor-Router'},'Location','southwest')
hold off
