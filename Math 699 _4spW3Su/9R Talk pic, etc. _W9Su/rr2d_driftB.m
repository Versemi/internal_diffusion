function [varargout] = rr2d_driftB(Nbugs,gridstruct0)
...
if nargin < 1, Nbugs = 10000; end

addpath rr2d

% Size of grid quadrant for a given number of bugs.
gridquadsize = @(Nbugs) 1.1*ceil(1.0276*(Nbugs^.6812));

if nargin < 2
  % No initial grid was specified, so make an empty one.
  Nbugs0 = 0;
  maxy = gridquadsize(Nbugs); miny = -5; 
  maxx = 2*ceil(Nbugs/maxy); minx = -maxx;
  ...
  maxy = gridquadsize(Nbugs0+Nbugs); miny = -5; 
  maxx = 2*ceil(Nbugs/maxy); minx = -maxx;
  x = minx:maxx; y = miny:maxy;
  grid = zeros(length(x), length(y));
